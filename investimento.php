...
if (isset($_POST['descricao']) && isset($_POST['valor']) && isset($_POST['data'])) {
    ...
    $sql = "INSERT INTO investimentos (descricao, valor, data, hora) VALUES ('$descricao', '$valor', '$data', '$hora')";
    if (!$mysqli->query($sql)) {
        $error_message = "Erro na inserção: " . $mysqli->error;
        error_log($error_message, 3, $log_file);
        die($error_message);
    }
    echo "Investimento registrado com sucesso.";
}
...
if (isset($_POST['update_id'])) {
    ...
    $sql = "UPDATE investimentos SET descricao='$update_descricao', valor='$update_valor', data='$update_data' WHERE id='$update_id'";
    if (!$mysqli->query($sql)) {
        $error_message = "Erro na atualização: " . $mysqli->error;
        error_log($error_message, 3, $log_file);
        die($error_message);
    }
    echo "Investimento atualizado com sucesso.";
}
...
if (isset($_POST['delete_id'])) {
    ...
    $sql = "DELETE FROM investimentos WHERE id='$delete_id'";
    if (!$mysqli->query($sql)) {
...
$mes_atual = date('Y-m');
$sql_total = "SELECT COUNT(*) AS total FROM investimentos WHERE DATE_FORMAT(data, '%Y-%m') = '$mes_atual'";
$result_total = $mysqli->query($sql_total);
$total_registros = $result_total->fetch_assoc()['total'];
$total_paginas = ceil($total_registros / $registros_por_pagina);

$sql = "SELECT id, descricao, valor, data, hora FROM investimentos WHERE DATE_FORMAT(data, '%Y-%m') = '$mes_atual' ORDER BY data DESC, hora DESC LIMIT $offset, $registros_por_pagina";
